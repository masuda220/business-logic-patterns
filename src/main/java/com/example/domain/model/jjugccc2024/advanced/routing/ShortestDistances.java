...
class ShortestDistances {
    ...
    private ShortestDistances(Place 出発地,
                              Map<Place, Integer> 各地点までの最短距離,
                              PathList 経路一覧,
                              RouteMap 経路マップ) {
        this.出発地 = 出発地;
    ...
    static ShortestDistances 初期化(Place 出発地, PathList 経路一覧, RouteMap 経路マップ) {
        Map<Place, Integer> 各地点までの最短距離 = new HashMap<>();
        各地点までの最短距離.put(出発地, 0); // 出発地から出発地の距離はゼロ

        return new ShortestDistances(出発地, 各地点までの最短距離, 経路一覧, 経路マップ);
    }
