...
class ActionsByStateTest {
    ...
    static ActionsByState 状態ごとのアクション = new ActionsByState();
    @Test
    void 可能なアクション() {
        assertEquals(Set.of(承認,差し戻し), 状態ごとのアクション.可能なアクション(審査中));
    }

    @Test
    void アクションの妥当性検査() {
        assertTrue(状態ごとのアクション.アクションの妥当性検査(審査中, 承認));
        assertFalse(状態ごとのアクション.アクションの妥当性検査(審査中, 取り下げ));
    }
}